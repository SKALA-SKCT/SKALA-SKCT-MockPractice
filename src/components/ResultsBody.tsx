...
/**
 * 결과 분석 본문(헤더 제외). 소유자 화면과 공유 화면이 함께 쓴다.
 * shared=true면 랭킹 카드를 숨긴다(코호트 집계는 로그인 소유자 화면에서만).
 */
export default function ResultsBody({ session, shared }: { session: Session; shared?: boolean }) {
  ...
  return (
    <>
      ...
      {!shared && <RankCard session={session} a={a} />}
